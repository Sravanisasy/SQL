...
SELECT S.State ,COUNT(DISTINCT(Parliament_Constituency)) AS TOTAL_SEATS  
FROM 
...
WHERE s.State = 'Andhra Pradesh' AND cr.Constituency_Name = 'ONGOLE';
...
SELECT TOP 10
    cr.Constituency_Name, cd.Candidate, cd.EVM_Votes 
FROM
    constituencywise_details cd
        JOIN
    constituencywise_results cr ON cd.Constituency_ID = cr.Constituency_ID
